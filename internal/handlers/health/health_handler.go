...
// handleCoopHealth returns the authenticated caller's own rolling coop
// health score - clientID/branchID are resolved once per request by
// middlewares.RequireMemberAuth, not re-resolved here.
func (h *HealthHandler) handleCoopHealth(ctx *gin.Context) {
	...
	score, err := h.healthService.CoopHealthService(ctx, clientID, branchID)
	if err != nil {
	...
	loggers.StatusOK(ctx, &common.SuccessResponse{
		...
		Status:     cc.SUCCESS_COOP_HEALTH_FETCHED,
