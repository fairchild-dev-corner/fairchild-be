...
// handleLastTransactions returns the authenticated caller's last 10
// loan/savings/time deposit/share capital postings - clientID/branchID are
// resolved once per request by middlewares.RequireMemberAuth, not
// re-resolved here.
func (h *TransactionsHandler) handleLastTransactions(ctx *gin.Context) {
	...
	txns, err := h.transactionsService.LastTransactionsService(ctx, clientID, branchID)
	if err != nil {
	...
	loggers.StatusOK(ctx, &common.SuccessResponse{
		...
		Status:     cc.SUCCESS_LAST_TRANSACTIONS_FETCHED,
